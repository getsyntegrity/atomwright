...
# Binary resolution
# ---------------------------------------------------------------------------
# The binary should be built and placed at /usr/local/bin/atomwright inside
# the Docker container. If not found, fall back to $HOME/atomwright or the
# current directory.
# Resolution priority (highest → lowest):
#   1. ./atomwright in the current repo directory (freshly built local binary)
#   2. ~/atomwright (explicit copy in home)
#   3. atomwright on PATH (system-installed, e.g. Homebrew)
# This ensures `go build ./cmd/atomwright && bash e2e/e2e_test.sh` always
# tests the locally built binary rather than the installed release version.
resolve_binary() {
    # Prefer the locally built binary (./atomwright) produced by `go build ./cmd/atomwright`.
    ...
    if [ -x "$repo_root/atomwright" ]; then
        echo "$repo_root/atomwright"
    elif [ -x "./atomwright" ]; then
        echo "./atomwright"
    elif [ -x "$HOME/atomwright" ]; then
        echo "$HOME/atomwright"
    elif command -v atomwright >/dev/null 2>&1; then
        echo "atomwright"
    else
        echo ""
    fi
...
cleanup_test_env() {
    ...
    rm -rf "$HOME/.atomwright" 2>/dev/null || true
