...
func RunReviewCaptureResult(args []string, stdout io.Writer) error {
	...
	input := flags.String("input", "", "raw reviewer result JSON file or - for stdin; `atomwright review schema reviewer` emits the schema and a working example")
	...
	if flags.NArg() != 0 || strings.TrimSpace(*lineage) == "" || strings.TrimSpace(*target) == "" ||
		...
		return reviewPreflightError(errors.New("review capture-result requires an exact repository context, --lineage, --target, --lens, --order, and either --input or --agent (or --preflight); `atomwright review status --contract gentle-ai.review-integration/v1 --next-transition` prints the exact bindings and `atomwright review schema reviewer` emits the result schema with a working example"))
	}
	...
	if *preflight && strings.TrimSpace(*input) == "" {
		if *subjectHash != "" && *subjectHash != subject.SubjectHash {
			...
			if legacyErr != nil {
				return reviewPreflightRefusal(reviewPreflightCaptureBindingMismatchReason, errors.New("review capture preflight subject hash does not match the provider-owned authority; refresh the binding with atomwright review status --cwd <repo> --contract <same-contract> --next-transition"))
			}
	...
	if providerExecution {
		...
		if *materialize {
			if *subjectHash != "" && *subjectHash != subject.SubjectHash {
				return reviewPreflightRefusal(reviewPreflightCaptureBindingMismatchReason, errors.New("materialize subject hash does not match the provider-owned authority; refresh the binding with atomwright review status --cwd <repo> --contract <same-contract> --next-transition"))
			}
