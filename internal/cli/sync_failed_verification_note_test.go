...
// The honest retry command for sync is simply `atomwright sync`.
func TestWithFailedSyncVerificationNoteNamesSyncCommand(t *testing.T) {
	...
	if !strings.Contains(updated.FinalNote, "atomwright sync") {
		t.Fatalf("FinalNote = %q, want it to name `atomwright sync`", updated.FinalNote)
	}
}
