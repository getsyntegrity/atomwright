...
// is nothing more to do from here" are wrong: `atomwright review mode
...
func TestReviewNarrationNeverClaimsNothingMoreToDo(t *testing.T) {
	...
	for code, statement := range reviewStopReasonNarration {
		...
		if strings.Contains(lowered, bannedClaim) {
			t.Errorf("stop reason %q narration still claims %q, which is false: `atomwright review mode disable` is always reachable: %q", code, bannedClaim, statement)
		}
	}
}
...
// name a real `atomwright` command / `--flag` of its own, or fall back to the
// universal self-service exit `atomwright review mode disable`
...
func TestReviewNarrationNamesAUniversalOrBetterExit(t *testing.T) {
	...
	for code, statement := range reviewStopReasonNarration {
		...
		if !namesOtherContinuation.MatchString(statement) {
			t.Errorf("stop reason %q narration names no runnable `atomwright` command, no `--flag`, and no %q fallback, so this stop reads as a dead end: %q", code, reviewConsentOffPathCommand, statement)
		}
	}
}
...
// `atomwright review mode disable` would let an orchestrator silently
...
func TestReviewNarrationReviewModeDisableIsAlwaysCloneScoped(t *testing.T) {
	re := regexp.MustCompile("`atomwright review mode disable[^`]*`")
	found := 0
	for code, statement := range reviewStopReasonNarration {
	...
	if found == 0 {
		t.Fatal("found no `atomwright review mode disable` invocations in reviewStopReasonNarration; the extraction is stale")
	}
}
...
// review_narration.go: `atomwright review status --next-transition` alone is
// refused by this real CLI without `--contract gentle-ai.review-integration/v2
// --agent claude-code`, and `atomwright review reopen-results` is refused
...
func TestReviewNarrationNamedCommandsAreAlwaysComplete(t *testing.T) {
	statusRe := regexp.MustCompile("`atomwright review status[^`]*--next-transition`")
	reopenRe := regexp.MustCompile("`atomwright review reopen-results[^`]*`")
	for code, statement := range reviewStopReasonNarration {
