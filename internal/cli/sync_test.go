...
import (
	...
	"github.com/pablogore/atomwright/v2/internal/components/communitytool"
	...
	"github.com/pablogore/atomwright/v2/internal/components/persona"
	"github.com/pablogore/atomwright/v2/internal/components/sdd"
	"github.com/pablogore/atomwright/v2/internal/model"
	opencodeconfig "github.com/pablogore/atomwright/v2/internal/opencode"
...
// finding 3: `atomwright sync --agent cluade` (a typo) previously printed
...
func TestParseSyncFlagsMistypedFlagNamesTheSupportedFlags(t *testing.T) {
	...
	if !strings.Contains(msg, "atomwright sync --help") {
		t.Fatalf("error = %q, want it to point at `atomwright sync --help`", msg)
	}
...
// TestParseSyncFlagsHelpFlagRendersUsage proves `atomwright sync --help` now
...
// surface audit finding 5: `atomwright sync claude` (a positional agent name
...
// Version, so every `atomwright sync` silently re-inflated the slim section
...
// but managed OpenCode plugins are already installed on disk, `atomwright sync`
...
func TestCodeGraphGuidanceSyncStepRefreshesOldMarkerWhenConfigured(t *testing.T) {
	...
	if !strings.Contains(text, "immediately run `atomwright codegraph init --cwd <project-root>`") || !strings.Contains(text, "custom notes") {
...
func TestCodeGraphGuidanceSyncStepRepairsCodexConfigOnlyGuidance(t *testing.T) {
	...
	text := string(body)
	for _, want := range []string{"<!-- gentle-ai:codegraph-guidance -->", "immediately run `atomwright codegraph init --cwd <project-root>`"} {
...
// atomwright sync excludes it from the plan and does not adopt it."
//
...
func TestRunSyncWithSelection_WritesExpectedFiles(t *testing.T) {
	...
	canonicalStatus := "atomwright review status --cwd <repo> --contract gentle-ai.review-integration/v2 --agent " + string(model.AgentOpenCode) + " --next-transition"
...
// by `atomwright sync --profile ...`, even though the sync reports success.
//
...
// `atomwright sync` preserves Custom per-phase Codex model assignments from
