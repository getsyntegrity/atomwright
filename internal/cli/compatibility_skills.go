...
func validateCompatibilityDestinations(root string, destinations []string) error {
	for _, destination := range destinations {
		...
		if err != nil || relative == "." || filepath.IsAbs(relative) || relative == ".." || strings.HasPrefix(relative, ".."+string(filepath.Separator)) {
			return fmt.Errorf("compatibility destination %q escapes %q; remove the escaping destination, then rerun atomwright install or atomwright sync", destination, root)
		}
		current := root
		for _, part := range strings.Split(filepath.Dir(relative), string(filepath.Separator)) {
			...
			if info.Mode()&os.ModeSymlink != 0 || !info.IsDir() {
				return fmt.Errorf("compatibility destination ancestor %q must be a physical directory; replace it with a physical directory, then rerun atomwright install or atomwright sync", current)
			}
		}
		info, err := lstatCompatibilityDestination(destination)
		if err == nil && (info.Mode()&os.ModeSymlink != 0 || !info.Mode().IsRegular()) {
			return fmt.Errorf("compatibility destination %q must be a regular file; replace it with a regular file or remove it, then rerun atomwright install or atomwright sync", destination)
		}
