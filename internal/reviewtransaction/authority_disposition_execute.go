...
func discoverAuthorityDispositionRecord(ctx context.Context, base, seed, planDigest string) (CompactReclaimRecord, bool, error) {
	...
	for _, entry := range entries {
		...
		if matched != nil {
			return CompactReclaimRecord{}, false, errors.New("authority disposition execution refused: duplicate quarantine records for the same plan digest; run `atomwright review inspect-authority` and escalate the report")
		}
		found := record
		matched = &found
	}
...
func resumeAuthorityDispositionRecord(ctx context.Context, record CompactReclaimRecord) (CompactReclaimRecord, error) {
	...
	if sourceExists == residueExists {
		return record, errors.New("authority disposition execution refused: ambiguous prepared residue state; run `atomwright review inspect-authority` and escalate the report")
	}
	if sourceExists {
...
func readBackAuthorityDisposition(ctx context.Context, root string, record CompactReclaimRecord) (CompactReclaimRecord, error) {
	if record.Status != CompactReclaimCommitted {
		return record, fmt.Errorf("authority disposition execution refused: readback observed a non-committed record; run `atomwright review inspect-authority --cwd %s` and escalate the report", pathquote.Quote(root))
	}
	...
	if !report.Complete {
		return record, fmt.Errorf("authority disposition execution refused: retained-graph readback is incomplete; run `atomwright review inspect-authority --cwd %s` and escalate the report", pathquote.Quote(root))
	}
	closureMembers := authorityDispositionClosureMembers(record)
	for _, edge := range report.Edges {
		if member := edge.PredecessorLineageID; closureMembers[member] {
			return record, fmt.Errorf("authority disposition execution refused: retained graph still references quarantined closure member %q; run `atomwright review inspect-authority --cwd %s` and escalate the report", member, pathquote.Quote(root))
		}
