...
func TestExecute_NoopWhenNothingIsExecutable(t *testing.T) {
	results := []update.UpdateResult{
		makeResult("atomwright", update.UpToDate, "1.0.0", "1.0.0", update.InstallBinary),
...
func TestExecute_DevBuildOnlyNoBackupCreated(t *testing.T) {
	...
	results := []update.UpdateResult{
		makeResult("atomwright", update.DevBuild, "dev", "1.0.0", update.InstallBinary),
	}
...
func TestExecute_DevBuildIsSkipped(t *testing.T) {
	...
	for i := range report.Results {
		if report.Results[i].ToolName == "atomwright" {
			r := report.Results[i]
			devResult = &r
		}
	}
