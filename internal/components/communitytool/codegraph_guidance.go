...
func CodeGraphGuidanceMarkdown() string {
	return strings.Join([]string{
		...
		"- Do not use `atomwright codegraph` as a general proxy. Its `init` command exists only to validate the project root before initialization; intelligence queries belong to the upstream CLI.",
		...
		"4. If `.codegraph/` is missing and CodeGraph is enabled/available, immediately run `atomwright codegraph init --cwd <project-root>` once.",
...
func hasCodeGraphGuidance(path string) bool {
	...
	return strings.Contains(content, "gentle-ai:"+codeGraphGuidanceSectionID) ||
		(strings.Contains(content, "codegraph") && strings.Contains(content, "atomwright codegraph init --cwd <project-root>"))
}
