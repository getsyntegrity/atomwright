...
func TestCodeGraphGuidanceContainsLazyInitAndUsageRules(t *testing.T) {
	...
	for _, want := range []string{
		...
		"immediately run `atomwright codegraph init --cwd <project-root>`",
		"atomwright codegraph init --cwd <project-root>",
		"codegraph_explore",
		...
		"`codegraph impact`",
		"`codegraph affected`",
		"Do not use `atomwright codegraph` as a general proxy",
...
func TestCodeGraphGuidanceInjectsForRepresentativeAgents(t *testing.T) {
	...
	for _, path := range []string{
		...
		text := string(content)
		if !strings.Contains(text, "<!-- gentle-ai:codegraph-guidance -->") || !strings.Contains(text, "atomwright codegraph init --cwd <project-root>") {
...
func TestCodeGraphGuidanceInjectRemovesLegacySkipBlock(t *testing.T) {
	...
	for _, want := range []string{"custom notes", "more notes", "<!-- gentle-ai:codegraph-guidance -->", "atomwright codegraph init --cwd <project-root>"} {
...
func TestCodeGraphGuidanceInjectRemovesUnmarkedUpstreamDuplicateBlock(t *testing.T) {
	...
	for _, want := range []string{"custom notes", "## CodeGraph manual notes", "This manual section is unrelated and must stay.", "more notes", "<!-- gentle-ai:codegraph-guidance -->", "atomwright codegraph init --cwd <project-root>"} {
		if !strings.Contains(text, want) {
			t.Fatalf("updated guidance missing %q:\n%s", want, text)
		}
	}
...
func TestCodeGraphGuidanceInjectPreservesManualNotesInsideUnmarkedCodeGraphSection(t *testing.T) {
	...
	for _, want := range []string{"custom notes", "Manual note: keep CodeGraph indexes outside throwaway directories.", "Manual note: rerun `codegraph sync` after large refactors.", "more notes", "<!-- gentle-ai:codegraph-guidance -->", "atomwright codegraph init --cwd <project-root>"} {
		if !strings.Contains(text, want) {
			t.Fatalf("updated guidance missing %q:\n%s", want, text)
		}
	}
...
func TestCodeGraphGuidanceInjectPreservesManualNoteBoundaryBeforeNextHeading(t *testing.T) {
	...
	for _, want := range []string{"custom notes", "This section must remain separate.", "<!-- gentle-ai:codegraph-guidance -->", "atomwright codegraph init --cwd <project-root>"} {
		if !strings.Contains(text, want) {
			t.Fatalf("updated guidance missing %q:\n%s", want, text)
		}
	}
...
func TestCodeGraphGuidanceInjectPreservesManualNotesBeforeUnmarkedUpstreamDuplicateBlock(t *testing.T) {
	...
	for _, want := range []string{"custom notes", "Manual note: always inspect the project root before using generated indexes.", "Manual note: never initialize CodeGraph in scratch directories.", "more notes", "<!-- gentle-ai:codegraph-guidance -->", "atomwright codegraph init --cwd <project-root>"} {
		if !strings.Contains(text, want) {
			t.Fatalf("updated guidance missing %q:\n%s", want, text)
		}
	}
...
func TestCodeGraphGuidanceInjectPreservesManualNotesInterleavedWithUnmarkedUpstreamDuplicateBlock(t *testing.T) {
	...
	for _, want := range []string{"custom notes", "Manual note: prefer the MCP tool when it returns exact source.", "Manual note: shell fallback is okay after CodeGraph initialization fails.", "more notes", "<!-- gentle-ai:codegraph-guidance -->", "atomwright codegraph init --cwd <project-root>"} {
		if !strings.Contains(text, want) {
			t.Fatalf("updated guidance missing %q:\n%s", want, text)
		}
	}
...
func TestInstallRefreshesOldCodeGraphGuidanceMarker(t *testing.T) {
	...
	if !strings.Contains(text, "immediately run `atomwright codegraph init --cwd <project-root>`") || !strings.Contains(text, "user content") {
