...
func TestInjectOpenCodeSingleModeSubagentPromptsOmitCodeGraphGuidanceByDefault(t *testing.T) {
	...
	for _, agentName := range sddInstalledSubAgentsForCodeGraphTest() {
		...
		if strings.Contains(prompt, "<!-- gentle-ai:codegraph-guidance -->") || strings.Contains(prompt, "atomwright codegraph init --cwd <project-root>") {
...
func TestInjectOpenCodeMultiModeSubagentPromptFilesIncludeCodeGraphGuidanceWhenEnabled(t *testing.T) {
	...
	if !strings.Contains(fixPrompt, "<!-- gentle-ai:codegraph-guidance -->") || !strings.Contains(fixPrompt, "atomwright codegraph init --cwd <project-root>") {
...
func TestInjectKimiYAMLSubagentsOmitCodeGraphGuidanceByDefault(t *testing.T) {
	...
	for _, fileName := range kimiYAMLSubagentFilesForCodeGraphTest() {
		...
		text := string(content)
		if strings.Contains(text, "  instructions: |-") || strings.Contains(text, "atomwright codegraph init --cwd <project-root>") {
...
func TestInjectKimiYAMLSubagentsRemainControlFilesWhenCodeGraphEnabled(t *testing.T) {
	...
	for _, fileName := range kimiYAMLSubagentFilesForCodeGraphTest() {
		...
		for _, forbidden := range []string{"  instructions: |-", "<!-- gentle-ai:codegraph-guidance -->", "atomwright codegraph init --cwd <project-root>"} {
		...
		if !strings.Contains(markdownText, "<!-- gentle-ai:codegraph-guidance -->") || !strings.Contains(markdownText, "atomwright codegraph init --cwd <project-root>") {
