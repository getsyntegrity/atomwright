/**
 * model-variants
 * Exports per-model variant (effort level) data for atomwright.
 *
 ...
 * ~/.atomwright/cache/model-variants.json. atomwright reads this file
...
export const ModelVariantsPlugin: Plugin = async (input) => {
  async function refreshVariantsCache() {
    ...
    try {
      ...
      const cacheDir = path.join(homedir(), ".atomwright", "cache")
