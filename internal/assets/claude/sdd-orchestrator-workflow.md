...
Before routing, continuing, applying, verifying, or archiving an SDD change, invoke the native dispatcher (`atomwright sdd-continue [change] --cwd <repo>` or `atomwright sdd-status [change] --cwd <repo> --json --instructions`). It resolves the artifact store the workspace declares and reports it in `artifactStore`.
