...
func TestVerificationIssuesMessageForCommandNamesTheGivenCommand(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			command: "atomwright install --agent claude-code",
			want:    "Installation completed with verification issues. Run `atomwright install --agent claude-code` to retry the failed checks.",
		},
		{
			name:    "sync retry command",
			command: "atomwright sync",
			want:    "Installation completed with verification issues. Run `atomwright sync` to retry the failed checks.",
		},
	}
