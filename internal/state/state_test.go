...
// Write/Read. Both `atomwright install` (CLI in run.go) and the TUI app
// (internal/app/app.go) write this field after a successful install so that
// `atomwright sync` regenerates the persona the user actually selected — not a
// hard-coded default.
...
func TestWriteCreatesStateDir(t *testing.T) {
	...
	if _, err := os.Stat(Root(home)); err != nil {
		t.Errorf("Write() did not create %q: %v", Root(home), err)
	}
}
...
func TestWriteStateFilePath(t *testing.T) {
	home := t.TempDir()
	got := Path(home)
	want := filepath.Join(home, ".atomwright", "state.json")
	if got != want {
...
func TestWriteFailurePreservesExistingState(t *testing.T) {
	...
	statePath := Path(home)
	stateTarget := filepath.Join(Root(home), "persisted-state.json")
