...
func TestArchitecture(t *testing.T) {
	specs.Describe(t, "the ADR-0001 dependency direction", func(s *specs.Spec) {
		s.When("the rules run against the real module", func(s *specs.Spec) {
			...
			//
			// internal/bootstrap and cmd/* are deliberately absent from the
			// expectation: #66 ATOM-BOOT-006 introduces them, and their rules
			// are proven by fixtures until then.
			s.It("still classifies a package into every layer the module is supposed to have", func(ctx *specs.Context) {
				...
				for _, l := range []layer{layerApplication, layerDomain, layerPlatform, layerAdapters} {
					if seen[l] == 0 {
