...
Use `atomwright skill-registry refresh` after you add, remove, rename, or move skills. Normal installs wire this refresh into startup hooks where the agent supports them, including Codex, Claude Code, OpenCode, and Pi through `gentle-pi`.
...
Audits each indexed SKILL.md against docs/skill-style-guide.md
   ...
   │
   ▼
Run atomwright skill-registry refresh again
```
...
```bash
atomwright skill-registry list          # name<TAB>scope<TAB>path
atomwright skill-registry list --json   # machine-readable, includes descriptions
```
