...
Run `atomwright review schema reviewer`, `atomwright review schema refuter`, or `atomwright review schema validator` to print a versioned JSON Schema. Each admitted capture is bound to its immutable candidate, revision, and selected role. Unknown fields and semantic violations are rejected before authority changes.
