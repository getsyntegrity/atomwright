...
| Platform | System Package Manager | Status |
|----------|----------------|--------|
...
| Windows 10/11 | winget | Supported (binary distribution held) |

The **System Package Manager** column is the manager Atomwright uses to install
*prerequisites* (`git`, `curl`, `node`), not a distribution channel for
Atomwright itself. Atomwright is installed on every platform by exactly two
supported paths — the curl install script and `go install` — and is not
published to Homebrew, Scoop, or any distro repository. See
...
When OpenCode background subagents are enabled through `atomwright install` or `atomwright sync`, Gentle AI™ writes only its own launcher files under `~/.atomwright/bin/`. POSIX systems use `~/.atomwright/bin/opencode`; Windows uses `opencode.cmd` and `opencode.ps1`. The launcher sets `OPENCODE_EXPERIMENTAL_BACKGROUND_SUBAGENTS=true` only when the variable is not already defined, so an explicit `false` always selects foreground execution.

Deactivation removes managed launcher files but may leave `~/.atomwright/bin/` in `PATH`; Gentle AI does not clean up shell profiles.
...
- **GGA on Windows** works from both Git Bash and PowerShell. atomwright installs a `gga.ps1` shim that automatically delegates to Git Bash, so no manual shell switching is required.
